...
window.VlocityDiagnosticWizard_ToolingAPI = {
  ...
  checkPicklistFieldValues: function (params) {
    ...
    try {
      ...
      const fieldValues = params.recordOptions.ToolingAPI.fieldValues || params.recordOptions.ToolingAPI.newFieldValues,
        updatedFieldValues = params.recordOptions.ToolingAPI.updatedFieldValues;

      const givenValues = (fieldValues ? fieldValues.map(val => typeof val === 'string' ? val : val.value) : null),
      ...
      const valuesNotIncluded = helperVal.valuesNotIncluded, 
        valueShouldBeDefault = helperVal.valueShouldBeDefault,
        valuesMustNotActive = helperVal.valuesMustNotActive;
      ...
      if(!Array.isArray(valuesMustNotActive) || valuesMustNotActive.length > 0){
        output.hasErrors = true;
        output.hasWarnings = true;
        output.errorMessage = output.errorMessage + (output.errorMessage.length != 0 ? ' ': '') +
          "The following picklist values must not be included in the active list: '" +
          valuesMustNotActive.join("', '") +
          "'.";
      }

      if(valueShouldBeDefault){
        output.hasErrors = true;
        output.hasWarnings = true;
        output.errorMessage = output.errorMessage + (output.errorMessage.length != 0 ? ' ': '') +
          "The picklist value '"+valueShouldBeDefault+"' should be set to default.";
      }
  ...
  /**
   * Adds and updates given Picklist field values via Tooling API on Apex
   * @param {Object} params
   * @returns {Promise}
   */
  setupPicklistFieldValues: function (params) {
    var output = {};
    ...
    try {
      ...
      const newFieldValues = params.recordOptions.ToolingAPI.newFieldValues,
        updatedFieldValues = params.recordOptions.ToolingAPI.updatedFieldValues,
        toolingResponse = JSON.parse(params.output.toolingResponse),
        toolingSubResult = toolingResponse.compositeResponse.pop();
      ...
      // Adding new picklist field values
      if(newFieldValues){
        const newFieldNames = [];
        newFieldValues.forEach(field =>{ newFieldNames.push(field.value)});

        const helperVal = VlocityDiagnosticWizard_ToolingAPI.picklistFieldValuesHelper({
          givenValues: newFieldNames,
          toolingResponse: toolingResponse,
          toolingSubResult: toolingSubResult,
          objectTableEnum: params.output.objectTableEnum
        });
        ...
        valuesNotIncluded.forEach(function (fieldValue) {
          let elem = params.recordOptions.ToolingAPI.newFieldValues.find(item => fieldValue === item.value);
  
          picklistValues.push({
            color: elem.color ? elem.color : null,
            default: elem.default != null ? elem.default : false,
            description: elem.description ? elem.description : null,
            isActive: elem.isActive != null ? elem.isActive : null,
            label: elem.label ? elem.label : elem.value,
            urls: elem.urls ? elem.urls : null,
            valueName: elem.value
          });
        });
      ...
      // Updating picklist field values
      if(updatedFieldValues){
        metadata.valueSet.valueSetDefinition.value.forEach(function (field) {
          let updatedElem = updatedFieldValues.find(item => item.value === field.valueName? item : null);

          if(updatedElem){
            if(updatedElem.isActive != null) {
              field.isActive = updatedElem.isActive;
            }

            if(updatedElem.label) {
              field.label = updatedElem.label;
            }

            if(updatedElem.description) {
              field.description = updatedElem.description;
            }

            if(updatedElem.urls) {
              field.urls = updatedElem.urls;
            }

            if(updatedElem.color) {
              field.color = updatedElem.color;
            }

            if(updatedElem.default != null) {
              if(updatedElem.default){
                // if updatedElem.default = true, unset current default picklist value first before setting a new default 
                metadata.valueSet.valueSetDefinition.value.forEach(function (fld) {
                  fld.default = false;
                });
              }
              field.default = updatedElem.default;
            }
          }
        });
      }
      ...
      output.hasErrors = true;
      output.errorMessage =
        "ToolingAPI.setupPicklistFieldValues: " + error.toString();
      return Promise.resolve(output);
    }
  },
  ...
  picklistFieldValuesHelper: function (params) {
    ...
    var valuesMustNotActive = [];
    var valueShouldBeDefault = null;
    if(params.updatedFieldValues){
      params.updatedFieldValues.forEach(function (updateItem) {
        
        if(updateItem.isActive != null && !updateItem.isActive){
          if(retrievedValuesMap.has(updateItem.value)){
            var retrievedPicklistValue = picklistValues.find(retrievedValue => retrievedValue.valueName == updateItem.value ? retrievedValue : null);

            // ACTIVE picklistValue has isActive = null OR isActive = true
            // INACTIVE picklistValue has isActive = false OR picklistValue is not included in the retrieved picklist values
            if(retrievedPicklistValue && (retrievedPicklistValue.isActive == null || retrievedPicklistValue.isActive)){
              valuesMustNotActive.push(updateItem.value);
            }
          } 
        }
        
        if(updateItem.default != null && updateItem.default) {
          var retrievedDefault = picklistValues.find(retrievedValue => retrievedValue.valueName == updateItem.value ? retrievedValue : null);

          if(retrievedDefault && !retrievedDefault.default){
            valueShouldBeDefault = updateItem.value;
          } 
        }
    ...
    return {
      ...
      valuesMustNotActive: valuesMustNotActive,
      valuesNotIncluded: valuesNotIncluded,
      valueShouldBeDefault: valueShouldBeDefault,
      output: output
    };
  },
